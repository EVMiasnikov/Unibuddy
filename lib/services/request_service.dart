...
class RequestService {
  ...
  Future<List<BuddyRequest>> getMyTasks(String buddyId) async {
    final snapshot = await _requests
        .where('acceptedBuddyId', isEqualTo: buddyId)
        .get()
        .timeout(const Duration(seconds: 10));

    final tasks = snapshot.docs
        .map((document) => BuddyRequest.fromMap(document.id, document.data()))
        .toList();
  ...
  Future<List<BuddyRequest>> getMyRequests(String requesterId) async {
    final snapshot = await _requests
    ...
    final requests = snapshot.docs.map((document) {
      return BuddyRequest.fromMap(document.id, document.data());
    }).toList();
  ...
  Future<List<BuddyRequest>> getPublicRequests(
    ...
    final buddyCity = currentUserCity.trim().toLowerCase();
    ...
    final requests = snapshot.docs
        ...
        .where((request) {
          ...
          // City must match.
          if (request.city.trim().toLowerCase() != buddyCity) {
            return false;
          }
          ...
          return request.targetBuddyId == currentUserId;
        })
        .toList();

    // Offers keep the most urgent tasks first.
    requests.sort((a, b) => a.dateTime.compareTo(b.dateTime));
  ...
  Future<void> acceptRequest({
    ...
    final requestReference = _requests.doc(requestId);
    ...
    try {
      await _db
          .runTransaction((transaction) async {
            ...
            final targetBuddyId = data['targetBuddyId'] as String?;

            // Will be used later when specified buddies are added.
            if (targetBuddyId != null && targetBuddyId != buddyId) {
              throw Exception('This request is intended for another buddy.');
            }

            transaction.update(requestReference, {
              'acceptedBuddyId': buddyId,
              'status': RequestStatus.accepted.name,
            });
          })
          .timeout(const Duration(seconds: 10));
    } on TimeoutException {
      throw Exception('Accepting the request timed out.');
    } on FirebaseException {
      rethrow;
    }
  }
    ...
    await _db
        .runTransaction((transaction) async {
          ...
          if (data['status'] != RequestStatus.accepted.name) {
            throw Exception('Only accepted requests can be cancelled.');
          }
